...
class Trainer:
    ...
    def _train_one_epoch(self, epoch, only_bert_train=False, is_diffusion_train=True):
        ...
        self.model.train()
        self.args.items_feature = self.args.items_feature.to(self.device)
        ...
        for batch in prog_iter:
            ...
            t, _ = self.schedule_sampler.sample(input_ids.shape[0], self.device)
            ...
            item_cl_loss = self.model.get_contrastive_loss(item_embeddings, torch.arange(self.args.item_size).to(self.device))
            ...
            tr_diff_nll_loss += diff_nll_loss / len(self.train_dataloader)
            tr_diff_mse_loss += diff_mse_loss / len(self.train_dataloader)
         
            tr_sas_rec_loss += sas_rec_loss / len(self.train_dataloader)
            tr_sas_cl_loss += sas_cl_loss / len(self.train_dataloader)
            tr_item_cl_loss += item_cl_loss / len(self.train_dataloader)
    ...
    def train(self):
        ...
        for epoch in trange(self.start_epoch, self.start_epoch + int(self.args.epochs), desc="Epoch"):
            ...
            scores, _ = self.eval(epoch, test=False)  #valid
        ...
        scores, _  = self.eval("best", test=True)  #test
