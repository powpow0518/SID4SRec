...
def info_nce(z_i, z_j, psi_seq, temp, batch_size, sim='dot'):
    """
    修改版的 info_nce 函數，加入相似度閾值 psi：
    當 z_i 和 z_j 的相似度大於等於 psi_seq 時，不將其視為負樣本
    只有當相似度小於 psi_seq 時，才視為負樣本
    """
    N = 2 * batch_size
    ...
    # 創建基本的負樣本掩碼
    mask = mask_correlated_samples(batch_size).to(positive_samples.device)
    
    # 根據相似度閾值 psi_seq 過濾負樣本
    # 創建一個新的掩碼，只有當相似度小於 psi_seq 時才視為負樣本
    psi_mask = (sim_matrix < psi_seq).to(positive_samples.device)
    # 結合原始掩碼和新的閾值掩碼
    filtered_sim = sim_matrix.clone()
    filtered_sim[~(mask & psi_mask)] = -1e9
...
#         # 計算負樣本數量
#         negatives_count = negative_samples.shape[1]
#         print(f"不使用 psi_seq: 負樣本數量 = {negatives_count}")
